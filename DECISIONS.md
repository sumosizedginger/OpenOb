...
## D-023 Unified Application Service & Single-Authority Gateway Model

**Decision:** All programmatic access to OpenOb from external agents (Hermes, Claude Code, Antigravity, Reasonix), local CLI scripts, and UI adapters is unified through a single application-service boundary: `OpenObWorkspace` (`@okw/workspace`). The local gateway (`apps/gateway`) is a lightweight Node.js HTTP server bound strictly to loopback (`127.0.0.1`) exposing read-only REST and MCP tool interfaces secured with Bearer token authentication. External callers are forbidden from directly manipulating the filesystem, bypassing `SafeWriter`, or mutating derived indexes independently. A vault operates in exactly one mode at a time: Browser-Direct Mode (UI owns FSA storage) or Gateway-Managed Mode (Gateway owns Node filesystem and coordinates state).

**Reason:** Prevents fragmented multi-writer concurrency hazards (`F-001`, `F-002`), guarantees atomic safe save semantics across all interfaces, and provides a stable, capability-gated foundation for external AI coding agents and tools.
