...
- Hardened `packages/index/src/query-engine.ts` with:
  ...
  - Type-safe scalar matching:
    - Numbers: Compared numerically (`10 > 2`), not lexicographically.
    - Dates: Validated with strict ISO-8601 date regex (`/^\d{4}-\d{2}-\d{2}/`) before numerical timestamp comparison.
    - Booleans: Parsed and compared as boolean primitives.
    - Arrays: Scalar elements checked for equality or containment.
    - YAML Maps/Objects: Protected against `[object Object]` stringification bugs and crashes.
...
- 9 tests covering folder scoping, numeric comparison, ISO date comparison, boolean filters, array containment, `is_empty`/`is_not_empty`, complex nested maps, deterministic sort tie-breaking, and pagination.

### 2. Differential Parity Suite (`tests/integrity/query-differential.test.ts`)
- 8 tests proving exact equivalence between `SqliteDocumentIndex` and `MemoryDocumentIndex` across all query operators and sorting configurations.
...
- 6 tests validating REST client, MCP tool execution, CLI JSON query, and CLI convenience flags against a live running Gateway.
...
- **Vitest Unit & Integrity**: 57 test files passed, 317 tests passed (0 failures).
